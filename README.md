...
> 注：百炼新用户赠送免费 token 额度，上述模型均适用。成本按 token 计费，`qwen3-vl-flash` 单次识图通常几分钱，免费额度足够日常使用很久（具体以百炼定价页为准）。模型以百炼控制台实际列表为准：如旧系列下线，把模型名换成控制台上现有的视觉模型即可（改 `.env` 的 `VISION_MODEL`，或直接对 AI 说"换个模型：xxx"）。
...
| 其他 | 控制台现有视觉模型 | 旧模型下线时，以百炼控制台实际列表为准 |
